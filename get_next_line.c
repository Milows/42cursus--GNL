...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/07 15:44:13 by micabrer          #+#    #+#             */
/*   Updated: 2023/06/12 13:23:43 by micabrer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	saved_data = read_save_data(fd, saved_data);
	if (!saved_data)
		return (NULL);
	line = extract_line(saved_data);
	saved_data = extract_remaining(saved_data);
	return (line);
}
